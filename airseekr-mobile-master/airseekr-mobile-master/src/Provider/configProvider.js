...
import { msgProvider, localStorage, msgTitle } from "./utilslib/Utils";
import {
  GoogleSignin,
...
const _BASE_URL = 'https://airseekr.com/';
const _BASE_URL_IMG = 'https://img.airseekr.com/';
class configProvider {

  baseURL = _BASE_URL + "app/webservice/";
  baseAPI = _BASE_URL + "api/book-a-stall/";
  baseAPI2 = _BASE_URL + "book-a-stall/";
  privateTokenKey = "yty456KEl23nwLo";
  img_url = _BASE_URL_IMG + "app/webservice/images/200X200/";
  img_url1 = _BASE_URL_IMG + "app/webservice/images/400X400/";
  img_url2 = _BASE_URL_IMG + "app/webservice/images/700X700/";
  img_url3 = _BASE_URL_IMG + "app/webservice/images/";
  login_type = "app";
  mapkey = "AIzaSyA53awfrjzwrm9U9SxrqtdXxtQkhuGUsRs";
  maplanguage = "ar";
  language = 0;
  player_id = "123456";
  player_id_me = "123456";
  ...
  AppLogout = async (navigation) => {
    ...
    try {
      // var queryOffinbox = firebase.database().ref("users/" + userdata.user_id + "/myInbox/");
      // queryOffinbox.off();
      // var updates = { "onlineStatus": "false" };
      // var onlineStatusRef = firebase.database().ref("users/" + userdata.user_id).update(updates);
      FirebaseInboxJson = [];
      count_inbox = 0;
      ...
      if (userdata != null) {
        if (userdata.login_type == "app") {
          ...
          console.log("face boook logout");
          LoginManager.logOut();
          console.log("face boook logout done");
          localStorage.clear();
          console.log("face boook logout done again");
          navigation.navigate("Login");
        ...
        } else if (userdata.login_type == "apple") {
